...
class Parameters:
    ...
    def read(self, parmfile):
        ...
        self.V2_NOISE = float(parms["V2_NOISE"].get_value(1))
...
class Agent:
  ...
  # Apply the indivudual variation (V2_NOISE parameter) to
  # the historical frequencies.
  def vary(self, freq_pair, parms):
    delta = int(parms.V2_NOISE * freq_pair.Total)
    delta = min(delta, freq_pair.V2)
    delta = min(delta, freq_pair.Total - freq_pair.V2)
    freq_pair.V2 += randrange(-delta, delta + 1)
    return freq_pair
  ...
  def generate_exemplars(self, parms):
    ...
    # Apply the indivudual variation (V2_NOISE parameter) to
    # the historical frequencies.
    individual_variation = FrequencyTable()
    individual_variation.Vf_THEN = self.vary(self.hist_freq.Vf_THEN.copy(), parms)
    individual_variation.Vf_AdvO = self.vary(self.hist_freq.Vf_AdvO.copy(), parms)
    individual_variation.Aux_THEN = self.vary(self.hist_freq.Aux_THEN.copy(), parms)
    individual_variation.Aux_AdvO = self.vary(self.hist_freq.Aux_AdvO.copy(), parms)

    self.utterances = FrequencyTable()  # The agent’s language model
    for i in range(parms.INITIAL_UTTERANCES):
      utterance = individual_variation.random_utterance()
  ...
  def get_doubt(self, parms):
    ...
    if av_P_V2 > 0.999: av_P_V2 = 0.999
