...
class Parameters:
    ...
    def read(self, parmfile):
        ...
        self.GEN_2_START = float(parms["GEN_2_START"].get_value(1))
        self.GEN_3_START = float(parms["GEN_3_START"].get_value(1))
...
def main():
  ...
  pop1_file = 'celt.txt'
  pop1_gen2_file = 'celt_gen2.txt'  #nog niet via cmd line
  pop1_gen3_file = 'celt_gen3.txt'  #nog niet via cmd line

  pop2_file = 'viking.txt'
  pop2_gen2_file = 'viking_gen2.txt'
  pop2_gen3_file = 'viking_gen3.txt'
  ...
  for i in range (0, parms.N_RUNS):
      ...
      for interaction_number in range(1, parms.NR_OF_INTERACTIONS + 1):
        ...
        if interaction_number/parms.NR_OF_INTERACTIONS > parms.GEN_2_START and current_generation == 1:
          Celt.hist_freq = read_frequencies(pop1_gen2_file)
          Viking.hist_freq = read_frequencies(pop2_gen2_file)

          # pop1_utterances = FrequencyTable()  #wat gebeurt hier?
          # pop2_utterances = FrequencyTable()
          current_generation = 2

        elif interaction_number/parms.NR_OF_INTERACTIONS > parms.GEN_3_START and current_generation == 2:
          Celt.hist_freq = read_frequencies(pop1_gen3_file)
          Viking.hist_freq = read_frequencies(pop2_gen3_file)
